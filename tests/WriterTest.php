...
require_once __DIR__ . '/TestXLSWriter.php';

class WriterTest extends \PHPUnit\Framework\TestCase
{
    /** @var XLSXWriter */
    private \TestXLSWriter $writer;
    ...
    {
        $header = ['year'=>'string', 'month'=>'string', 'amount'=>'money', 'first_event'=>'datetime', 'second_event'=>'date'];
        ...
        $this->assertSame(1000, $this->writer->getTabRatio());

        $this->writer->setTabRatio(500);
        $this->assertSame(500, $this->writer->getTabRatio());
        ...
        $this->writer->writeSheetRow("Sheet1", ['2003', '3', '33.3', '2023-02-01 00:00:00', '2012-12-31 00:00:00'], ['hidden' => false]);
        ...
        $this->assertSame(4, $this->writer->countSheetRows("Sheet1"));

        $this->assertSame("test", XLSXWriter::sanitize_filename("t<>e?\"s:/\\|*&t".chr(0)));
    ...
    public function testFormars(): void
    {
        ...
        $this->writer->writeSheetRow('BasicFormats',[$pi, $pi, $pi, $pi, $pi, $pi, $pi, $pi, $pi] );
        $this->writer->writeSheetHeader('Dates',$sheet2header);
        $this->writer->writeSheetRow('Dates',[$date, $date, $date, $date, $date, $date, $date, $date, $date, $date, $time, $time] );
        $this->writer->writeSheetHeader('Currencies',$sheet3header);
        $this->writer->writeSheetRow('Currencies',[$amount, $amount, $amount, $amount, $amount, $amount, $amount, $amount, $amount] );
    ...
    {
        $styles1 = ['font'=>'Arial', 'font-size'=>10, 'font-style'=>'bold', 'fill'=>'#eee', 'halign'=>'center', 'border'=>'left,right,top,bottom'];
        $styles2 = [['font-size'=>6], ['font-size'=>8], ['font-size'=>10], ['font-size'=>16]];
        $styles3 = [['font'=>'Arial'], ['font'=>'Courier New'], ['font'=>'Times New Roman'], ['font'=>'Comic Sans MS']];
        $styles4 = [['font-style'=>'bold'], ['font-style'=>'italic'], ['font-style'=>'underline'], ['font-style'=>'strikethrough']];
        $styles5 = [['color'=>'#f00'], ['color'=>'#0f0'], ['color'=>'#00f'], ['color'=>'#666']];
        $styles6 = [['fill'=>'#ffc'], ['fill'=>'#fcf'], ['fill'=>'#ccf'], ['fill'=>'#cff']];
        $styles7 = ['border'=>'left,right,top,bottom'];
        $styles8 = [['halign'=>'left'], ['halign'=>'right'], ['halign'=>'center'], ['halign'=>'none']];
        $styles9 = [[], ['border'=>'left,top,bottom'], ['border'=>'top,bottom'], ['border'=>'top,bottom,right']];


        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles1 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles2 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles3 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles4 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles5 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles6 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles7 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles8 );
        $this->writer->writeSheetRow('Sheet1', [300, 234, 456, 789], $styles9 );
    ...
    public function testColors(): void
    {
        ...
        foreach($colors as $b) {
            foreach($colors as $g) {
                ...
                $rowstyle = [];
                foreach($colors as $r) {
                    $rowdata[] = sprintf('#%s%s%s', $r, $g, $b);
                    $rowstyle[] = ['fill'=>sprintf('#%s%s%s', $r, $g, $b)];
                }
    ...
    {
        $header = ["col1"=>"string", "col2"=>"string", "col3"=>"string", "col4"=>"string"];

        $this->writer->writeSheetHeader('Sheet1', $header, $col_options = ['widths'=>[10,20,30,40]] );
        $this->writer->writeSheetRow('Sheet1', $rowdata = [300, 234, 456, 789], $row_options = ['height'=>20] );
        $this->writer->writeSheetRow('Sheet1', $rowdata = [300, 234, 456, 789], $row_options = ['height'=>30] );
        $this->writer->writeSheetRow('Sheet1', $rowdata = [300, 234, 456, 789], $row_options = ['height'=>40] );
    ...
    {
        $keywords = ['some', 'interesting', 'keywords'];
        ...
        $rows = [["Merge Cells Example"], [100, 200, 300, 400, 500], [110, 210, 310, 410, 510]];
        ...
        $rows = [
            ['Spreadsheet', '_'],
            ["Hoja de cálculo", "Hoja de c\xc3\xa1lculo"],
            ["Електронна таблица", "\xd0\x95\xd0\xbb\xd0\xb5\xd0\xba\xd1\x82\xd1\x80\xd0\xbe\xd0\xbd\xd0\xbd\xd0\xb0 \xd1\x82\xd0\xb0\xd0\xb1\xd0\xbb\xd0\xb8\xd1\x86\xd0\xb0"],
            //utf8 encoded
            ["電子試算表", "\xe9\x9b\xbb\xe5\xad\x90\xe8\xa9\xa6\xe7\xae\x97\xe8\xa1\xa8"],
        ];

        $this->writer->writeSheet($rows, $sheet2);

        $sheet3 = 'fonts';
        $format = ['font'=>'Arial', 'font-size'=>10, 'font-style'=>'bold,italic', 'fill'=>'#eee', 'color'=>'#f00', 'fill'=>'#ffc', 'border'=>'top,bottom', 'halign'=>'center'];
        $this->writer->writeSheetRow($sheet3, $row=[101, 102, 103, 104, 105, 106, 107, 108, 109, 110], $format);
        $this->writer->writeSheetRow($sheet3, $row=[201, 202, 203, 204, 205, 206, 207, 208, 209, 210], $format);

        $sheet4 = 'row_options';
        $this->writer->writeSheetHeader($sheet4, ["col1"=>"string", "col2"=>"string"], $col_options = ['widths'=>[10,10]] );
        $this->writer->writeSheetRow($sheet4, [101, 'this text will wrap'], $row_options = ['height'=>30, 'wrap_text'=>true]);
        $this->writer->writeSheetRow($sheet4, [201, 'this text is hidden'], $row_options = ['height'=>30, 'hidden'=>true]);
        $this->writer->writeSheetRow($sheet4, [301, 'this text will not wrap'], $row_options = ['height'=>30, 'collapsed'=>true]);
    ...
    public function testAutoFilter(): void
    {
        ...
        $header = ['col-string'=>'string', 'col-numbers'=>'integer', 'col-timestamps'=>'datetime'];

        $this->writer->writeSheetHeader('Sheet1', $header, ['auto_filter'=>true, 'widths'=>[15,15,30]] );
        for($i=0; $i<1000; $i++)
        {
            $this->writer->writeSheetRow('Sheet1', [str_shuffle($chars), mt_rand()%10000, date('Y-m-d H:i:s',time()-(mt_rand()%31536000))]);
        }
